...
public class Shierlukeyboard extends AppCompatActivity implements View.OnTouchListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        waveformslider.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                wave = progress;
                waveToast.waveinfo(progress, Shierlukeyboard.this);
            }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            tones[tone-1] = soundgenerator.generateTone(shierlu[tone-1], volume, wave, this, tones, tone-1);
            tones[tone-1].play();
            lastfreq = BigDecimal.valueOf(shierlu[tone - 1]).setScale(4, BigDecimal.ROUND_FLOOR) + " Hz";
